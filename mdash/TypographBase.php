 ...
 * 
 * Refactoring to run under PHP 7.4
...
class TypographBase
{
	...
	public function safe_blocks($text, $way, $show = true)
	{
		if (count($this->_safe_blocks)) {
			$safeType = true === $way
				? fn ($m) => Lib::encrypt_tag($m[2])
				: fn ($m) => stripslashes(Lib::decrypt_tag($m[2]));
	...
	public function get_style($list = false, $compact = false)
	{
		...
		foreach ($this->trets as $tret) {
			...
			foreach ($arr as $classname => $str) {
				...
				$clsname = ($this->class_layout_prefix ? $this->class_layout_prefix : "")
					. (isset($this->tret_objects[$tret]->class_names[$classname]) ? $this->tret_objects[$tret]->class_names[$classname] : $classname);
				$res[$clsname] = $str;
			}
		}
		if ($list) return $res;
	 ...
	 * @param int $layout
	 */
	public function set_tag_layout(int $layout = Lib::LAYOUT_STYLE)
	{
	 ...
	 *
	 * @param array $map
	 * @param bool $disable если ложно, то $map соотвествует тем правилам, которые надо включить иначе это список правил, которые надо выключить
	...
	{
		if (!is_array($setupmap)) {
			return;
		}

		if (isset($setupmap['map']) || isset($setupmap['maps'])) {

			// Непонятный код, явно с ошибками. Закоментил. Kort.
