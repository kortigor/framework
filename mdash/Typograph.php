...
class Typograph extends TypographBase
{
	...
	protected $all_options = array(
		...
		'Space.clear_before_after_punct' => array(
		...
		'Etc.unicode_convert' => array(
			'description' => 'Преобразовывать html-сущности в юникод',
			'selector' => array('*', 'Etc.nobr_to_nbsp'),
			'setting' => array('dounicode', 'active'),
			'exact_selector' => true,
			'disabled' => true
		),
	...
	public function get_options_list()
	{
		...
		foreach ($this->group_list as $group => $ginfo) {
			if ($ginfo === true) {
				...
				if ($tret) $info['title'] = $tret->title;
				else $info['title'] = "Не определено";
			} else {
				$info = $ginfo;
			}
			$info['name'] = $group;
