...
class Lib
{
	...
	public static $_charsTable = array(
		'"' => array(
			'html' => array('&laquo;', '&raquo;', '&rdquo;', '&lsquo;', '&bdquo;', '&ldquo;', '&quot;', '&#171;', '&#187;'),
	...
	public static function safe_tag_chars($text, $way)
	{
		if ($way) {
			$callback = function ($m) {
				$need = (strlen($m[1]) == 1 && substr(trim($m[2]), 0, 1) == '-' && substr(trim($m[2]), 1, 1) != '-') ? true : false;
				return $need ? $m[0] : $m[1] . (substr(trim($m[2]), 0, 1) === "a" ? "%%___"  : "") . static::encrypt_tag(trim($m[2])) . $m[3];
			};
		} else {
			$callback = function ($m) {
				$need = (strlen($m[1]) == 1 && substr(trim($m[2]), 0, 1) == '-' && substr(trim($m[2]), 1, 1) != '-') ? true : false;
				return $need
					? $m[0]
					: $m[1] . (substr(trim($m[2]), 0, 3) === "%%___" ? static::decrypt_tag(substr(trim($m[2]), 4)) : static::decrypt_tag(trim($m[2]))) . $m[3];
			};
		}

		$text = preg_replace_callback('/(\<\/?)([^<>]+?)(\>)/s', $callback, $text);
		return $text;
	}
	...
	{
		$text = preg_replace_callback('/' . static::INTERNAL_BLOCK_OPEN . '([a-zA-Z0-9\/=]+?)' . static::INTERNAL_BLOCK_CLOSE . '/s', function ($m) {
			return static::decrypt_tag($m[1]);
		}, $text);
		return $text;
	}
	...
	public static function convert_html_entities_to_unicode(&$text)
	{
		$text = preg_replace_callback("/\&#([0-9]+)\;/", function ($m) {
			return static::_getUnicodeChar(intval($m[1]));
		}, $text);

		$text = preg_replace_callback("/\&#x([0-9A-F]+)\;/", function ($m) {
			return static::_getUnicodeChar(hexdec($m[1]));
		}, $text);
