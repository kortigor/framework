...
use core\activeform\InputWidget as BaseInputWidget;

/**
 * InputWidget is the base class for widgets extending the [[BaseInputWidget]] that collect user inputs in all
 ...
 *
 * Classes extending from this widget can be used in an [[\core\activeform\ActiveForm|ActiveForm]] using the
 * [[\core\activeform\ActiveField::widget()|widget()]] method. For example like this:
 *
 * ```php
 ...
 */
class InputWidget extends BaseInputWidget implements BootstrapInterface
{
    use WidgetTrait;
    ...
    /**
     * @var bool whether input is to be disabled
     */
    public bool $disabled = false;

    /**
     * @var bool whether input is to be readonly
     */
     ...
     */
    public bool $pluginLoading = true;
    ...
    /**
     * @var bool whether the widget should automatically format the date from the PHP DateTime format to the
     * javascript/jquery plugin format. This is more applicable for widgets that manage date / time inputs.
     *
     * @see http://php.net/manual/en/function.date.php
     */
    public bool $convertFormat = false;
    ...
    protected function initInputWidget()
    {
        if (!isset($this->language)) {
            $this->language = Sys::$app->language;
        }
    ...
    protected function parseDateFormat($type)
    {
        ...
        $attrib = $type . 'Format';
        $format = isset(f()->$attrib) ? f()->$attrib : '';
        if (empty($format)) {
