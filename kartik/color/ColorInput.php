...
{
    /**
     * @var bool whether to use the native HTML5 color input
     */
     ...
     */
    public bool $showDefaultPalette = true;

    /**
     * @var string the name of the jQuery plugin
     */
    public string $pluginName = 'spectrum';
    ...
    public function run()
    {
        ...
        if (substr($this->language, 0, 2) !== 'en') {
            $this->_defaultOptions += [
                'cancelText' => t('Отмена'),
                'chooseText' => t('Выбрать'),
                'clearText' => t('Сбросить выбор цвета'),
                'noColorSelectedText' => t('Цвет не выбран'),
                'togglePaletteMoreText' => t('Ещё'),
                'togglePaletteLessText' => t('Скрыть'),
            ];
        }
