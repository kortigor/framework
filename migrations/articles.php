...
$tableName = 'articles';
$tableNameT = $tableName . '_translations';
$tableNameC = $tableName . '_category';
$tableNameCT = $tableName . '_category_translations';

Capsule::schema()->dropIfExists($tableNameCT);
Capsule::schema()->dropIfExists($tableNameT);
Capsule::schema()->dropIfExists($tableName);
Capsule::schema()->dropIfExists($tableNameC);

// Articles category
Capsule::schema()->create($tableNameC, function (Blueprint $table) {
...
echo sprintf('Table "%s" created successfully', $tableNameC) . "\n";

// Articles category translate
Capsule::schema()->create($tableNameCT, function (Blueprint $table) use ($tableNameC) {
    $table->id();
    ...
    $table->unique(['language', 'parent_id']);
    $table->foreign('parent_id')->references('id')->on($tableNameC)->onDelete('cascade')->onUpdate('cascade');
});

echo sprintf('Table "%s" created successfully', $tableNameCT) . "\n";

// Articles
Capsule::schema()->create($tableName, function (Blueprint $table) use ($tableNameC) {
    $table->uuid('id')->primary('id');
    $table->string('slug')->unique();
    $table->uuid('category_id');
    $table->tinyInteger('status')->unsigned();
    $table->tinyInteger('allow_comments')->unsigned();
    $table->smallInteger('order')->unsigned();
    $table->timestamps();

    $table->foreign('category_id')->references('id')->on($tableNameC)->onDelete('cascade')->onUpdate('cascade');
});

echo sprintf('Table "%s" created successfully', $tableName) . "\n";

// Articles translate
Capsule::schema()->create($tableNameT, function (Blueprint $table) use ($tableName) {
    $table->id();
    $table->char('language', 10);
    ...
    $table->unique(['language', 'parent_id']);
    $table->foreign('parent_id')->references('id')->on($tableName)->onDelete('cascade')->onUpdate('cascade');
});

echo sprintf('Table "%s" created successfully', $tableNameT) . "\n";

$idc = \core\entities\Id::next()->getId();

\customer\entities\ArticleCategory::insert(
    [
        [
            'id' => $idc,
            'slug' => 'cat-slug',
...
\customer\entities\translations\ArticleCategory::insert(
    [
        [
            'language' => 'ru',
            'name' => 'Общие вопросы',
...
$id = \core\entities\Id::next()->getId();

\customer\entities\Article::insert(
    [
        [
            'id' => $id,
            'category_id' => $idc,
...
\customer\entities\translations\Article::insert(
    [
        [
            'language' => 'ru',
            'title' => 'Вопрос',
            'announce' => 'Что это такое?',
            'text' => 'Это страшная вещь!',
            'parent_id' => $id,
            'created_at' => new \DateTime(),
            'updated_at' => new \DateTime(),
        ],
