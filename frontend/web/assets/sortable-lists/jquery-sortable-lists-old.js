		...
		return this.on( 'mousedown', function( e )
			{
				...
				if( state.isDragged !== false ) return;
				if( setting.handlerClass && !target.hasClass( setting.handlerClass ) ) return; // setting.handlerClass is checked cause hasClass('')
				if( setting.ignoreClass && target.hasClass( setting.ignoreClass) ) return; // setting.ignoreClass is checked cause hasClass('') returns true
