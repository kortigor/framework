...
(function ($) {
	...
	$.fn.sortableLists = function (options) {
		...
		return this.on('mousedown', function (e) {
			...
			// Fix by Kort, to be able to include to ignore all items in ignoreclass container
			if (
				state.isDragged !== false
				|| (setting.ignoreClass && target.hasClass(setting.ignoreClass))
				|| (setting.ignoreClass && target.parents('.' + setting.ignoreClass).length)
	 ...
	 * @returns string
	 */
	$.fn.sortableListsToString = function (expression, arr, parentId) {
		expression || /(.+)[-=_](.+)/; // Default for integer id
		arr = arr || [];
		...
		$(this).children('li').each(function () {
			var li = $(this),
				id = li.attr('id'),
				// Fix by Kort to be able to pass custom match pattern, to use uuid
				// matches = id ? id.match( /(.+)[-=_](.+)/ ) : null; // string "0" is evaluate to true but is not valid
				// matches = id ? id.match(/(.+)_(.+)/) : null; // string "0" is evaluate to true but is not valid for uuid
				matches = id ? id.match(expression) : null; // string "0" is evaluate to true but is not valid
			...
			$(this).children('ul,ol').sortableListsToString(expression, arr, matches[2]);
