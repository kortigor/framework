 ...
 *
 * LinkPager works with a [[\core\data\Pagination]] object which specifies the total number
...
class LinkPager extends Widget
{
    ...
    /**
     * @var bool Hide first/last page links if they in pageRange
     * @see getPageRange() method
     */
    public bool $smartFirstLastPages = true;
    /**
     * @var bool Hide prev/next page links if current page first or last
     */
    public bool $smartPrevNextPages = true;
     ...
     * These links are generated using [[\core\data\Pagination::getLinks()]].
     * @see http://www.w3.org/TR/html401/struct/links.html#h-12.1.2
     */
    protected function registerLinkTags()
    {
        ...
        foreach ($this->pagination->getLinks() as $link) {
            foreach ($link->getRels() as $rel) {
                $view->registerLinkTag(['rel' => $rel, 'href' => $link->getHref()]);
            }
        }
    }
    ...
    protected function renderPageButtons()
    {
        ...
        // first page
        $firstPageLabel = $this->firstPageLabel === true ? '1&nbsp;&hellip;' : $this->firstPageLabel;
        $firstSmartShow = $this->smartFirstLastPages && $this->getPageRange()[0] > 0;
        if ($firstPageLabel !== false && $firstSmartShow) {
        ...
        // prev page
        $prevHide = $this->smartPrevNextPages && $currentPage === 0;
        if ($this->prevPageLabel !== false && $pageCount > $this->maxButtonCount && !$prevHide) {
        ...
        // next page
        $nextHide = $this->smartPrevNextPages && $currentPage === $endPage;
        if ($this->nextPageLabel !== false && $pageCount > $this->maxButtonCount && !$nextHide) {
        ...
        // last page
        $lastPageLabel = $this->lastPageLabel === true ? '&hellip;&nbsp;' . $pageCount : $this->lastPageLabel;
        $lastSmartShow = $this->smartFirstLastPages && $this->getPageRange()[1] < ($pageCount - 1);
        if ($lastPageLabel !== false && $lastSmartShow) {
            $buttons[] = $this->renderPageButton(
                $lastPageLabel,
                $pageCount - 1,
