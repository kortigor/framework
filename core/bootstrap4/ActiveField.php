...
/**
 * A Bootstrap 4 enhanced version of [[\core\activeform\ActiveField]].
 *
 * This class adds some useful features to [[\core\activeform\ActiveField|ActiveField]] to render all
 ...
 * and radioLists in the [[\core\activeform\ActiveForm::fieldConfig|fieldConfig]] of the
 * [[\core\activeform\ActiveForm]]:
 *
 ...
 * ```
 *
 * @property-read \core\bootstrap4\ActiveForm $form
 *
 * @see \core\bootstrap4\ActiveForm
 * @see http://getbootstrap.com/css/#forms
 *
 * @author Michael Härtl <haertl.mike@gmail.com>
 * @author Simon Karlen <simi.albi@outlook.com>
 */
class ActiveField extends \core\activeform\ActiveField
{
    /**
     ...
     * {@inheritdoc}
     */
    public array $options = ['class' => ['widget' => 'form-group']];
    /**
     * {@inheritdoc}
     */
    public array $inputOptions = ['class' => ['widget' => 'form-control']];
    /**
    ...
    /**
     * @var array the default options for the input radios. The parameter passed to individual
     * input methods (e.g. [[fileCustomInput()]]) will be merged with this property when rendering the input tag.
     *
     * If you set a custom `id` for the input element, you may need to adjust the [[$selectors]] accordingly.
     *
     * @see \core\helpers\Html::renderTagAttributes() for details on how attributes are being rendered.
    ...
    public array $fileOptions = [
        'class' => ['widget' => 'custom-file-input'],
        'labelOptions' => [
            'class' => ['widget' => 'custom-file-label'],
            'data-browse' => 'Browse'
        ]
    ];
    /**
     * {@inheritdoc}
     */
    public array $errorOptions = ['class' => 'invalid-feedback'];
    /**
     * {@inheritdoc}
     */
     ...
     * {@inheritdoc}
     */
    public array $hintOptions = ['class' => ['widget' => 'form-text', 'text-muted'], 'tag' => 'small'];
    /**
    ...
    /**
     * @var string the template for files
     */
    public string $fileTemplate = "<div class=\"custom-file\">\n{input}\n{label}\n{error}\n{hint}\n</div>";
    /**
     ...
     * {@inheritdoc}
     */
    public function checkbox(array $options = [], bool $enclosedByLabel = false): static
    {
     ...
     * {@inheritdoc}
     */
    public function radio(array $options = [], bool $enclosedByLabel = false): static
    {
    ...
    public function checkboxList(array $items, array $options = []): static
    {
        if (!isset($options['item'])) {
            ...
            $options['item'] = function ($i, $label, $name, $checked, $value) use (
                ...
                // Added by Kort, to fix bug and correctly display errors
                $this->addErrorClassIfNeeded($options);
     ...
     */
    public function staticControl(array $options = [])
    {
     ...
     */
    protected function createLayoutConfig(array $instanceConfig)
    {
        $config = [
            'hintOptions' => [
                'tag' => 'small',
    ...
    /**
     * Custom file input. Added by Kort.
     * 
     * @param array $options
     * 
     * @return $this the field object itself.
     */
    public function fileCustomInput($options = [])
    {
        $fileOptions = $this->fileOptions;
        $options = ArrayHelper::merge($fileOptions, $options);
        Html::removeCssClass($options, 'form-control');
        $labelOptions = ArrayHelper::remove($options, 'labelOptions', []);
        $wrapperOptions = ArrayHelper::remove($options, 'wrapperOptions', []);
        $this->labelOptions = ArrayHelper::merge($this->labelOptions, $labelOptions);
        ...
        return parent::fileInput($options, false);
    }
