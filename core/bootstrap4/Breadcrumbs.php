...
class Breadcrumbs extends Widget
{
     ...
     * If this property is not set, it will default to a link pointing to `main.homeUrl` setting
    ...
    /**
     * @var bool Render last crumb as link.
     */
    public bool $activeLastLink = false;
    ...
    public function run()
    {
        ...
        if ($this->homeLink === null) {
            $links[] = $this->renderItem([
                'label' => 'Home',
                'url' => $this->getRequest()->getAttribute('homeUrl'),
            ], $this->itemTemplate);
        } elseif ($this->homeLink !== false) {
            $links[] = $this->renderItem($this->homeLink, $this->itemTemplate);
        }

        $totalLinks = count($this->links);
        foreach ($this->links as $ind => $link) {
            if (!is_array($link)) {
                $link = ['label' => $link];
            }
            if (($ind + 1) === $totalLinks && !$this->activeLastLink) {
                ArrayHelper::remove($link, 'url');
            }
