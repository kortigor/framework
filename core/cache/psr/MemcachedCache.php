...
use core\cache\exception\CacheMemcachedException;
 ...
 * 
 * @link https://github.com/yiisoft/cache-memcached Forked from
 */
final class MemcachedCache implements PsrSimpleCacheInterface
{
    ...
    private function initServers(array $servers, string $persistentId): void
    {
        ...
        if (!$this->cache->addServers($servers)) {
            throw new CacheMemcachedException('An error occurred while adding servers to the server pool.');
        }
    }
