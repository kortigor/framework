...
$records = new Records;
$out = fopen(__DIR__ . '/../Relations.php', 'w');
$compiler->compile($records(), $out, 'Relations', 'core\\link');
// }
...
class Records
{
    ...
    {
        $ch = curl_init(self::REGISTRY_FILE);
...
class RegistryCompiler
{
     ...
     *
     * @param iterable<SimpleXMLElement> $records An iterable of SimpleXml elements from the Link Relations XML file.
     * @param $stream An open file stream to which to write the generated code.
     * @param string $class The name of the interface to produce.
     * @param string $namespace The namespace of the interface to generate.
     */
    public function compile(iterable $records, $stream, string $class, string $namespace): void
    {
...
END;
     ...
     *
     * @param SimpleXMLElement $xref The xref argument of a record.
     * @return string A web URL.
     * @throws InvalidArgumentException
     */
    protected function xrefLink(SimpleXMLElement $xref): string
    {
        $type = (string) $xref['type'];
        return match ($type) {
            'uri' => (string) $xref['data'],
            'rfc' => 'https://tools.ietf.org/html/' . (string) $xref['data'],
            default => throw new InvalidArgumentException("Unhandled type: {$type}")
        };
    }
     ...
     *
     * @param string $text The string to rewrap.
     * @return string The rewrapped string.
     */
     ...
     *
     * @param string $value The rel name from the record.
     * @return string A const-name-safe string.
     */
     ...
     *
     * @param string $class The name of the class/interface to generate.
     * @param string $namespace The namspace for the generated class.
     * @return string A portion of a valid PHP class file.
     */
