 ...
 * ```
 * echo $form->field($model, 'from_date')->widget(\core\activeform\MaskedInput::class, [
 *     'mask' => '999-999-9999',
 * ])
 * ```
 *
...
class MaskedInput extends InputWidget
{
     ...
     */
    public string|array|JsExpression $mask;
    /**
    ...
    public function init()
    {
        ...
        if (empty($this->mask) && empty($this->clientOptions['alias'])) {
            throw new InvalidConfigException(
                "Either the 'mask' property or the 'clientOptions[\"alias\"]' property must be set."
            );
        }
    }
    ...
    protected function initClientOptions()
    {
        ...
        foreach ($options as $key => $value) {
            if (!$value instanceof JsExpression && in_array($key, [
