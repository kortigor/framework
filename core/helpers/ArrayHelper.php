...
class ArrayHelper
{
    ...
    /**
     * In relation to `$value`, selects:
     *  - nearest higher;
     *  - nearest lower;
     *  - nearest by value (default);
     * number in the array.
     * 
     * @param array $arr Values array
     * @param mixed $value Value 
     * @param int $method Method to check
     * 
     * @return mixed nearest value depending of method.
     */
    public static function getNearest(array $arr, $value, int $method = self::NEAREST_DEFAULT)
    {
        if (!count($arr)) {
            return null;
        }
    ...
    public static function getValue($array, $key, $default = null)
    {
        ...
        if (is_object($array)) {
            ...
            // by Kort
            // really it's possible to check by Reflection whether property is exists and accessible beforehand.
            $reflection = new ReflectionObject($array);
            try {
                $property = $reflection->getProperty($key);
                return $property->isPublic() ? $array->$key : $default;
            }
            // Property not exist
            catch (\ReflectionException $e) {
                // try to rely on `__get()` magic method if exists,
                // and here expected to fail if magic method can not correctly handle it
                if (method_exists($array, '__get')) {
                    return $array->$key;
                }
                return $default;
            }
     ...
     * ```
     * array(
     * 		array('key'=> 'abc', 'value' => 'qwerty'),
     * 		array('key'=> 'abc2', 'value' => 'qwerty2')
     * 	);
     * ```
     * 
     * To array:
     ...
     * @param boolean $rev Defines conversion order 'qwerty'=>'abc' or 'abc'=>'qwerty'
     * 
     * @return array
     * @author Kort
     */
    public static function reduceKeys(array &$a, bool $rev = true): array
    {
        if ($rev) {
            $k = 1;
            $v = 0;
    ...
    /**
     * Remove duplicates in associative array
     *
     * @param  array $rows
     *
     * @return array
     */
    public static function uniqueAssociative(array $rows): array
    {
        if (empty($rows)) {
            return $rows;
        }
    ...
    /**
     * Check string occurrence in array of strings
     *
     ...
     * @return bool
     */
    public static function strpos(array $arr, string $str): bool
    {
        $res = array_filter($arr, fn ($value) => mb_strpos($str, (string) $value) !== false);
        return !empty($res);
    }

    /**
     * Version of `array_shift()` function to work with associative arrays.
     * 
     * @param mixed $arr
     * @return array|null first array element in key=>value terms
     * @see array_shift()
     */
    public static function shiftAssociative(&$arr): ?array
    {
        if (!is_array($arr) || empty($keys = array_keys($arr))) {
            return null;
        }
        $k = $keys[0];
        $r = [$k => $arr[$k]];
    ...
    /**
     * Shift first element from array and place result to an array.
     * 
     * With indexed:
     * ```
     * $arr = ['first', 'second', 'third'];
     * ArrayHelper::shiftFirst($arr); // ['first']
     * ```
     * With associative:
     * ```
     * $arr = ['first' => 'one', 'second' => 'two', 'third' => 'three'];
     * ArrayHelper::shiftFirst($arr); // ['first' => 'one']
     * ```
     * 
     * @param array $arr
     * 
     * @return array|null
     */
    public static function shiftFirst(array &$arr): ?array
    {
        reset($arr);
        ...
        $value = is_int($key) ? [$arr[$key]] : [$key => $arr[$key]];
        unset($arr[$key]);
        return $value;
    }
}