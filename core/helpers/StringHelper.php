...
/**
 * Concrete implementation for StringHelper.
 *
...
class StringHelper
{
    ...
    public static function generateRandomString(int $length = 32)
    {
        ...
        $hex = bin2hex($bytes);
        $hash = hash('sha512', $hex . time());
        return substr(StringHelper::base64UrlEncode($hash), 0, $length);
    }

    /**
     * Generates a random alpha string of specified length.
     * 
     * @param int $length
     * 
     * @return string
     */
    public static function randomStringAlpha(int $length = 32): string
    {
     ...
     * 
     * Features:
     * - the integrity of words is preserved;
     * - punctuation marks at the end of the line are removed.
     * 
     * @param string $string Given string.
     * @param int $length Desired string length.
     * @param bool $hellip Add '...' to the end or no.
     * 
     * @return string
     */
    public static function trimLine(string $string, int $length = 650, bool $hellip = true): string
    {
        $string = trim($string);
        $symb = [',', '.', ':', ';', '-'];
        ++$length;
        if ($length && mb_strlen($string) > $length) {
            ...
            if (in_array(mb_substr($str, -1), $symb)) {
    ...
    /**
     * Trim the string to the desired length, adding '...' at the end.
     * Useful for the links with too long text.
     * 
     ...
     * @return string
     */
    public static function trimLink(string $text, int $length): string
    {
        $text = trim($text);
        $dec = ["&", "\"", "'", "\\", '\"', "\'", "<", ">"];
        $enc = ["&amp;", "&quot;", "&#39;", "&#92;", "&quot;", "&#39;", "&lt;", "&gt;"];
    ...
    /**
     * Custom transliteration. Converts сyrillic 'Привет' to 'Privet' or 'privet'.
     *
     * @param string $string Given string.
     * @param bool $lower Convert to lowercase or no.
     *
     * @return string Transliterated string.
     */
    public static function translit(string $string, bool $lower = true): string
    {
        if ($lower) {
        ...
        $latin = [
            ...
            'a', 'b', 'v', 'g', 'd', 'e', 'zh', 'z', 'i', 'k', 'l', 'm', 'n', 'o', 'p', 'r', 's', 't', 'u', 'f', 'h', 'ts', 'ch', 'sh', 'shch', 'e', 'yu', 'ya',
            'a', 'b', 'v', 'g', 'd', 'e', 'e', 'z', 'i', 'y', 'k', 'l', 'm', 'n', 'o', 'p', 'r', 's', 't', 'u', 'f', 'h',
            'j', 'i', 'e', '_', 'zh', 'ts', 'ch', 'sh', 'shch', '', 'yu', 'ya'
        ];

        $string = str_replace($cyr, $latin, $string);
     ...
     * @return string
     */
    public static function removeNl(string $string): string
    {
        return str_replace(["\n", "\r", "\n\r", "\r\n", "\t"], '', $string);
    }
}