 ...
 *
 * Attention: do not confuse [[\core\bootstrap4\Html]], be careful in which class
...
class Html
{
     ...
     * @param string|null $content the content to be decoded
     ...
     *  - visible: specifies condition when tag not displayed (value equal to `false`)
     *
    ...
    public static function beginForm(string $action = '', string $method = 'post', array $options = []): string
    {
        ...
        $request = Sys::$app->getController()->getRequest();
        if ($csrfName = $request->getAttribute('csrfParam')) {
            $csrf = ArrayHelper::remove($options, 'csrf', true);
            if ($csrf && strcasecmp($method, 'post') === 0) {
                $hiddenInputs[] = static::hiddenInput($csrfName, $request->getAttribute($csrfName));
            }
        }
     ...
     */
    public static function img($src, array $options = []): string
    {
        $options['src'] = $src;
     ...
     * [[\core\helpers\ArrayHelper::map()]].
     *
     * Note, the values and labels will be automatically HTML-encoded by this method, and the blank spaces in
     * the labels will also be HTML-encoded.
     * @param array $options the tag options in terms of name-value pairs. The following options are specially handled:
     *
     * - placeholder: string, a placeholder text to be displayed as hint if no option is selected. You can use an array
     ...
     *   ```
     * 
     * - prompt: string, a prompt text to be displayed as the first option. You can use an array
     *   to override the value and to set other tag attributes:
     * 
     *   ```php
     *   ['text' => 'Please select', 'options' => ['value' => 'none', 'class' => 'prompt', 'label' => 'Select']],
     ...
     * - encodeSpaces: bool, whether to encode spaces in options prompt, placeholder and option value with `&nbsp;` character.
     *   Defaults to false.
     * - encode: bool, whether to encode options prompt, placeholder and option value characters.
     *   Defaults to `true`.
     *
     ...
     */
    public static function activeHint(Model $model, string $attribute, array $options = []): string
    {
        $attribute = static::getAttributeName($attribute);
     ...
     * @return array of the validation errors
     */
    private static function collectErrors($models, bool $encode, bool $showAllErrors): array
    {
        $lines = [];
        if (!is_array($models)) {
            $models = [$models];
     ...
     * @return string the generated input tag
     */
    public static function activeInput(string $type, Model $model, string $attribute, array $options = []): string
    {
    ...
    /**
     * If `maxlength` option is set true and the model attribute is validated by a
     * `Webmozart\Assert\Assert::maxlength` validator which have signature `maxLength($value, $max, $message = '')`,
     * the `maxlength` option will take the value of argument `$max`.
     * @param Model $model the model object
     * @param string $attribute the attribute name or expression.
     * @param array $options the tag options in terms of name-value pairs.
     * @see \core\validators\Assert::maxlength()/
     */
    private static function normalizeMaxLength(Model $model, string $attribute, array &$options): void
    {
        if (isset($options['maxlength']) && $options['maxlength'] === true) {
            unset($options['maxlength']);
            $attrName = static::getAttributeName($attribute);
            foreach ($model->getActiveValidators($attrName) as $validator) {
                if ($validator->getName() === 'maxLength') {
                    $options['maxlength'] = $validator->arguments[0];
                    break;
                }
            }
        }
     ...
     *   by a string validator, the `maxlength` option will take the value of `Webmozart\Assert\Assert::maxlength`
     *   validator which have signature `maxLength($value, $max, $message = '')`, the `maxlength` option will take the value of argument `$max`.
     ...
     * the attributes of the resulting tag. The values will be HTML-encoded using [[encode()]].
     */
    protected static function setActivePlaceholder(Model $model, string $attribute, array &$options = []): void
    {
     ...
     * @return string the generated input tag
     */
    public static function activeHiddenInput(Model $model, string $attribute, array $options = []): string
    {
     ...
     */
    public static function activeTextarea(Model $model, string $attribute, array $options = []): string
    {
        $name = $options['name'] ?? static::getInputName($model, $attribute);
     ...
     * @return string the generated radio button tag
     */
    public static function activeRadio(Model $model, string $attribute, array $options = []): string
    {
     ...
     * @return string the generated checkbox tag
     */
    public static function activeCheckbox(Model $model, string $attribute, array $options = []): string
    {
     ...
     */
    protected static function activeBooleanInput(string $type, Model $model, string $attribute, array $options = []): string
    {
        $name = $options['name'] ?? static::getInputName($model, $attribute);
        $value = static::getAttributeValue($model, $attribute);
     ...
     * @return string the generated drop-down list tag
     */
    public static function activeDropDownList(Model $model, string $attribute, array $items, array $options = []): string
    {
        if (empty($options['multiple'])) {
     ...
     * @return string the generated list box tag
     */
    public static function activeListBox(Model $model, string $attribute, array $items, array $options = []): string
    {
     ...
     * @return string the generated checkbox list
     */
    public static function activeCheckboxList(Model $model, string $attribute, array $items, array $options = []): string
    {
     ...
     * @return string the generated radio button list
     */
    public static function activeRadioList(Model $model, string $attribute, array $items, array $options = []): string
    {
     ...
     */
    protected static function activeListInput(string $type, Model $model, string $attribute, array $items, array $options = []): string
    {
        $name = $options['name'] ?? static::getInputName($model, $attribute);
     ...
     * This method will take out these elements, if any: "prompt", "placeholder", "options" and "groups". See more details
     * in [[dropDownList()]] for the explanation of these elements.
     *
     * @return string the generated list options
     * @author Kort <kort.igor@gmail.com> in scope of adding "placeholder" option.
     */
    public static function renderSelectOptions($selection, array $items, array &$tagOptions = []): string
    {
        ...
        if (isset($tagOptions['placeholder'])) {
            $placeholderOptions = ['value' => '', 'style' => 'display:none;'];
            if (is_string($tagOptions['placeholder'])) {
                $placeholderText = $tagOptions['placeholder'];
            } else {
                $placeholderText = $tagOptions['placeholder']['text'];
                $placeholderOptions = array_merge($placeholderOptions, $tagOptions['placeholder']['options']);
            }
            $placeholderText = $encode ? static::encode($placeholderText) : $placeholderText;
            if ($encodeSpaces) {
                $placeholderText = str_replace(' ', '&nbsp;', $placeholderText);
            }
            $lines[] = static::tag('option', $placeholderText, $placeholderOptions);
        }
        ...
        unset($tagOptions['prompt'], $tagOptions['options'], $tagOptions['groups'], $tagOptions['placeholder']);
     ...
     * `'data' => ['params' => ['id' => 1, 'name' => 'somename']]` would be rendered as
     * `data-params='{"id":1,"name":"somename"}'`.
     *
    ...
    /**
     * Converts and merge options to an array.
     * 
     * You can give options as string:
     * ```
     * $opts = 'param1="value1"';
     * ```
     * or as array:
     ...
     *
     * If an attribute value is an instance of [[ActiveRecord]] or an array of such instances,
     * the primary value(s) of the AR instance(s) will be returned instead.
     *
     * @param ModelValidableInterface $model the model object
     * @param string $attribute the attribute name or expression
     * 
     * @return string|array the corresponding attribute value
     * @throws InvalidArgumentException if the attribute name contains non-word characters.
     */
    public static function getAttributeValue(ModelValidableInterface $model, string $attribute)
    {
        if (!preg_match(static::$attributeRegex, $attribute, $matches)) {
            throw new InvalidArgumentException('Attribute name must contain word characters only.');
        }
     ...
     * @throws InvalidArgumentException if the attribute name contains non-word characters.
     */
    public static function getInputName(Model $model, string $attribute): string
    {
     ...
     * @throws InvalidArgumentException if the attribute name contains non-word characters.
     */
    public static function getInputId(Model $model, string $attribute): string
    {
        $charset = 'UTF-8';
     ...
     * Render header tag
     * 
     * @param int $level Header level (h1, h2...)
     * @param string $text Header text
     * @param array $options H tag rendering options.
     * 
     * @return string
     */
    public static function h(int $level, string $text, array $options = []): string
    {
        $tag = 'h' . (string) $level;
        $content = static::encode(rtrim($text, '.'));
        return static::tag($tag, $content, $options);
    }

    /**
     * Render header tag with link
     * 
     * @param int $level Header level (h1, h2...)
     * @param string $text Header text
     ...
     * @param array $options H tag rendering options.
     * The following options are specially handled:
     *  - url, array link ('a' tag) rendering options
     * 
     * @return string
     */
    public static function hLink(int $level, string $text, string $url, array $options = []): string
    {
        $tag = 'h' . (string) $level;
        $urlOptions = ArrayHelper::remove($options, 'url', []);
        $content = static::encode(rtrim($text, '.'));
        $content = static::a($content, $url, $urlOptions);
        return static::tag($tag, $content, $options);
    }
}