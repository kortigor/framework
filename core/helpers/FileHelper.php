...
class FileHelper
{
     ...
     *
     * - Convert all directory separators into `DS` (e.g. "\a/b\c" becomes "/a/b/c")
     ...
     * @param string $ds the directory separator to be used in the normalized result. Defaults to `DS`.
     * @return string the normalized file/directory path
     */
    public static function normalizePath(string $path, string $ds = DS)
    {
     ...
     * @throws InvalidArgumentException when the `fileinfo` PHP extension is not installed and `$checkExtension` is `false`.
     */
    public static function getMimeType($file, $magicFile = null, $checkExtension = true)
    {
        ...
        if (!extension_loaded('fileinfo')) {
            ...
            throw new InvalidArgumentException('The fileinfo PHP extension is not installed.');
        }
     ...
     */
    protected static function loadMimeTypes($magicFile = null)
    {
        if ($magicFile === null) {
            $magicFile = static::$mimeMagicFile;
        }
     ...
     * @return array the mapping from file extensions to MIME types
     */
    protected static function loadMimeAliases($aliasesFile = null)
    {
        if ($aliasesFile === null) {
    ...
    public static function copyDirectory($src, $dst, $options = [])
    {
        ...
        if ($src === $dst || strpos($dst, $src . DS) === 0) {
     ...
     */
    public static function generateName(int $length = 25): string
    {
        return substr(md5(strval(rand(111111, 999999))), 0, $length);
    }

    /**
     * Checks that file exists in specified directory.
     * 
     * @param string $dir Directory where to check file.
     * @param string $file File name.
     * @param string $prefix Prefix for file name (used only if file exists).
     * 
     * @return string File name exists in directory:
     * - no: returns given file name without changes;
     * - yes: generate new random file name and return it.
     */
    public static function fileExists(string $dir, string $file, string $prefix = ''): string
    {
        $ext = static::getExtension($file);
        while (is_file(static::normalizePath($dir . DS . $file))) {
            $file = $prefix . static::generateName() . $ext;
        }
        return $file;
    }
    ...
    /**
     * Performs check that file is image
     * 
    ...
    public static function isImage(string $file): bool
    {
        ...
        $mime = static::getMimeTypeByExtension($file);
        return strstr($mime, 'image') === false ? false : true;
    }

    /**
     * Create normalized file path for file placed in given directory
     * 
     * @param string $dir Directory where file placed in.
     * @param string $file File name.
     * 
     * @return string
     */
    public static function getFilePath(string $dir, string $file): string
    {
        return static::normalizePath($dir . DS . $file);
    }
